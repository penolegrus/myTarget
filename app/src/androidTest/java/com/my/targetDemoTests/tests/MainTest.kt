...
class MainTest : TestBase() {
    ...
    /**
     * Добавляет новый юнит с определенным именем и slot id. Проверяет, что юнит добавлен
     */
    @Test
    fun test_addNewUnit() {
        val slot = 10
        val name = "Test new Unit"
        MainScreen().addUnit(slot, name)
        assertDisplayed(name)
    }

    /**
     * Добавляет новый юнит без slot id. Проверяет что юнит не добавлен и окно с добавлением все еще видно
     */
    @Test
    fun test_addNewUnitWithoutSlot() {
        MainScreen().addUnitWithoutSlot()
        assertContains("slot id");
    }
        ...
        fun test_RemoveUnit() {
            ...
            onView(withId(R.id.main_recycler)).perform(
                actionOnItemAtPosition<androidx.recyclerview.widget.RecyclerView.ViewHolder>(
                    unitsBefore - 1, GeneralSwipeAction(
